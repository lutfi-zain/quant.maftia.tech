...
- **Single API Gateway Encorcement (`api.quant.maftia.tech` / `:8765`):** Do not spin up ad-hoc temporary backend servers on random ports (`:3000`, `:8766`, etc.) in production. All cross-system data querying and live WebSocket broadcasts must route strictly through the unified Hono v4 + Bun API Gateway.
