...
- **`ValuationComposite` (`valuation_composite`):** Macroeconomic cycle valuation score piecewise linear interpolated into `[-2.0, +2.0]` across 17 Fundamental, Technical, and Sentiment indicators. Acts as the primary macro `CircuitBreakerFilter` when `score <= -1.50` (bubble risk) or `>= +1.00` (deep discount). Powers the **SDCA Strategy Engine** with Bayesian Optuna-optimized thresholds, rolling 4-year dynamic percentile rescaling, and a continuous transaction ledger (`action, price, amount, balance`).
- **`LTTDRegime` (`lttd_regime`):** Orthogonal long-term trend classification via 3-State Gaussian HMM (`BULL`, `BEAR`, `SIDEWAYS`) using Log Returns and 20-day Volatility, validated by PCA and VIF pruning ($>10$). Governed by **v3.3 HL-driven execution** ($HL=200\text{d}$): asymmetric SuperSmoother 35/20, MHP 60d, RCO 30d, Trend MA 250d, Macro Breakdown Emergency Exit Gate (`smoothed_score_exit <= -0.10` AND `regime == BEAR`), rolling 750d dynamic quantile thresholds (65/35 with 0.30/0.22 fallback), and dual-mode support (`LTTD_MODE=macro` vs `LTTD_MODE=weeks`). When `regime == SIDEWAYS` ($P_{\text{Sideways}} > 0.60$), acts as a macro override forcing `0.0` exposure on mid-term trend systems.
...
- **Master Unified Architecture Specification:** [[00_unified_architecture.md]](file:///home/ubuntu/projects/quant.maftia.tech/docs/architecture/00_unified_architecture.md)
- **Orchestration & Data Sync Pipeline:** [[run_report_pipeline.py]](file:///home/ubuntu/projects/run_report_pipeline.py)
- **Valuation System Architecture:** [[01_valuation_system.md]](file:///home/ubuntu/projects/quant.maftia.tech/docs/architecture/01_valuation_system.md)
- **LTTD System Architecture:** [[02_lttd_system.md]](file:///home/ubuntu/projects/quant.maftia.tech/docs/architecture/02_lttd_system.md)
- **MTTD System v2 Architecture:** [[03_mttd_system.md]](file:///home/ubuntu/projects/quant.maftia.tech/docs/architecture/03_mttd_system.md)
- **Ichimoku Quant Architecture:** [[04_ichimoku_system.md]](file:///home/ubuntu/projects/quant.maftia.tech/docs/architecture/04_ichimoku_system.md)
...
- **Single API Gateway Enforcement & Network Binding (`api.quant.maftia.tech` / `:8910`):** Do not spin up ad-hoc temporary backend servers or use default/random ports (`:3000`, `:8000`, `:8080`, etc.) in production or development. All cross-system data querying and live WebSocket broadcasts must route strictly through the unified Hono v4 + Bun API Gateway on port `:8910`. Always explicitly bind server listeners (`serve()`, `app.listen()`, `export default { fetch, port, hostname }`) to `0.0.0.0` (`hostname: '0.0.0.0'`) so services are accessible externally and across containers rather than restricted to `127.0.0.1` or `localhost`.
...
- **Multi-File Reference Cleanliness (`2026-07-08`):** When modifying across multiple Markdown specs (`README.md`, `docs/architecture/*.md`), always use exact line ranges and run `git diff` after edits to verify that section numbering and markdown tables remain aligned without duplicate headers. — [Evidence: 4-system cleanup across `README.md` and canonical `docs/architecture/` files]
- **Chart Sizing & Crosshair Lock Enforcement (`2026-07-08`):** When building frontend subplots (`Lightweight Charts v5.2`), any chart without an explicit right Y-axis width lock (`85px`) and vertical crosshair sync will experience horizontal time-tick drift when price ($60k+) vs oscillator ($-1.0$) character widths differ. Always verify `85px` lock on every subplot container. — [Evidence: `docs/architecture/00_unified_architecture.md` & Phase 2 user interview confirmation]
...
- **LTTD Exposure Column Sync (`2026-07-13`):** Subsystem model target exposures (like `lttd_exposure` in `unified_daily_analytics`) must be explicitly extracted from the daily subsystem databases and updated in the main sync script `run_report_pipeline.py`. Leaving these column values unpopulated defaults them to `NULL` in the database, causing the frontend components to fall back to simplified regime heuristics and breaking continuous backtesting. — [Evidence: `run_report_pipeline.py` LTTD target_exposure sync addition]
- **Continuous Sizing Backtests (`2026-07-13`):** The quantitative terminal studios (specifically `LttdLab.tsx`) must bind their strategy backtest positions directly to continuous target exposure columns (`lttd_target_exposure` / `lttd_exposure`) rather than binarizing them (e.g. `rawExposure > 0 ? 1 : 0`). Sizing strategies rely on continuous scaling, and utilizing the raw exposure increases model performance validation from a 0.46 Sharpe ratio (under the old regime heuristic) to a 1.32 Sharpe ratio (under the actual target exposure strategy). — [Evidence: `LttdLab.tsx` and `verify_lttd_studio_metrics_1to1.py` actual target exposure updates]
- **SDCA Ledger Architecture (`2026-07-19`):** The SDCA (Dollar Cost Averaging) backtest engine (`calculate_sdca_backtest.py`) outputs a continuous transaction ledger (`action: BUY/SELL`, `price`, `amount`) rather than matched trade pairs (`entryPrice`, `exitPrice`). Frontend UI tables (like in `ValuationStudio.tsx`) must strictly map to this ledger format. Never attempt to read `entryPrice` or `exitPrice` from SDCA trades to avoid `toLocaleString` unhandled exceptions that unmount the React tree. — [Evidence: UI crash fixed in `ValuationStudio.tsx` table rendering]
- **OpenSpec Archive Strict Header Matching (`2026-07-19`):** When running `openspec archive`, the CLI requires exact markdown header string matches for `MODIFIED` blocks. If the main spec header was renamed or doesn't exist, the command will abort. Use `openspec archive --skip-specs -y <change>` to bypass spec synchronization when forcing an archive of a completed implementation. — [Evidence: `optimize-sdca-buy-sell-conditions` archive failure due to missing header]
