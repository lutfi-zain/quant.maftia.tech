...
The API Gateway route handler (`src/api/routes/components.ts`) SHALL normalize incoming `system` or `system_source` query parameter strings (`quant-btc-valuation-system`, `quant-btc-lttd-system`, `quant-btc-mttd-system`, `quant-lttd-ichimoku`, or their short domain names) into exact upper-case canonical database identifiers (`VALUATION`, `LTTD`, `MTTD`, `ICHIMOKU`) before querying the `unified_component_signals` table.
...
- **WHEN** a client sends a GET request to `/api/v1/quant/components?system=quant-lttd-ichimoku`
