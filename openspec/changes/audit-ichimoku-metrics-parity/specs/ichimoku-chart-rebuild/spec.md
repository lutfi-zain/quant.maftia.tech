...
**CHANGED**: The IchimokuTerminal SHALL receive Tenkan-sen, Kijun-sen, Senkou Span A, Senkou Span B, and Chikou Span as API-served data fields from the unified daily analytics endpoint. The prior client-side computation function `computeIchimokuLines()` using hardcoded periods (9, 26, 52) SHALL be removed and replaced with data from the API response `ichimoku_imo.tenkan`, `ichimoku_imo.kijun`, `ichimoku_imo.senkou_a`, `ichimoku_imo.senkou_b`, `ichimoku_imo.chikou`.
...
All line styling (colors, widths, opacity) SHALL remain unchanged. Only the data source and periods change.

#### Scenario: Ichimoku lines render on BTC pane from API data

- **WHEN** IchimokuTerminal loads with daily data that includes `ichimoku_imo.tenkan`, `ichimoku_imo.kijun`, `ichimoku_imo.senkou_a`, `ichimoku_imo.senkou_b`, `ichimoku_imo.chikou` fields
...
- **THEN** the Chikou Span (purple, `rgba(168,85,247,0.55)`) represents the prior system's chikou value (60-bar displacement, Ehlers SuperSmoother filtered) as returned by the API

#### Scenario: Graceful fallback when API Ichimoku lines are NULL

- **WHEN** the API response returns null for `ichimoku_imo.tenkan`, `ichimoku_imo.kijun`, etc. (during warmup period at start of history)
...
The IchimokuTerminal S-Component pane (Pane 3, bottom subplot) SHALL render S_TK, S_Cloud, S_Future, and S_Chikou lines using values from the `ichimoku_imo.s_tk`, `ichimoku_imo.s_cloud`, `ichimoku_imo.s_future`, `ichimoku_imo.s_chikou` fields in the daily API response. The existing synthetic fallback code (`p.ichimoku_imo * 0.8`, `Math.sin(i * 0.08) * 0.6`, etc.) SHALL be removed entirely. If an API value is NULL for a given date (warmup period), that individual date point SHALL be omitted rather than replaced with a synthetic value.

#### Scenario: S-component lines render with real API data

- **WHEN** IchimokuTerminal loads daily data containing `ichimoku_imo.s_tk`, `ichimoku_imo.s_cloud`, `ichimoku_imo.s_future`, `ichimoku_imo.s_chikou`
