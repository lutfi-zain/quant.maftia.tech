...
### Requirement: Binance Adapter OHLCV Fetch Path
**Reason**: The `BinanceAdapter` class in `engines/lttd/src/data/exchange_adapter.py` and the `BTC_DATA_SOURCE=binance` env-var toggle are deprecated. All OHLCV data is sourced from `bitview.space` exclusively. The `EXCHANGE_API_KEY` env-var (used only by `BinanceAdapter`) is also removed.
**Migration**: Set `BTC_DATA_SOURCE` env-var is no longer needed. Remove it from any `.env` files or deployment configuration. The LTTD pipeline automatically uses `BRKExchangeAdapter` (bitview.space).
