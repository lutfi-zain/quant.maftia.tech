...
Defines requirements for ensuring $1:1$ performance metric parity, trade execution log parity, causal $T-1$ position shifting, 85px Y-axis width locking, vertical crosshair synchronization, and interactive frontend feature parity across all 4 quantitative defense studios (`Valuation Studio`, `LTTD Lab`, `MTTD Console`, and `Ichimoku Terminal`) against their prior canonical engines under `/home/ubuntu/projects/quant*`.
