...
## Purpose
Defines requirements for automated Playwright verification testing (`/web/tests/chart-sync.spec.ts`), checking `rightPriceScale: { minimumWidth: 85 }` enforcement on all chart subplots, and verifying bidirectional vertical crosshair alignment across multi-pane layouts.
