...
lttdRouter.get("/history", (c) => {
    ...
    `;
		...
		const rows = executeQuery<any>(sql, [
    ...
    `;
		...
		return c.json(
			rows.map((row: any) => {
				...
				if (
					row.lttd_prob_bull !== null &&
					row.lttd_prob_bear !== null &&
					row.lttd_prob_sideways !== null
				) {
					return {
						date: row.date,
						regime,
...
lttdRouter.get("/diagnostics", (c) => {
    ...
    `;
		const signals = executeQuery<any>(signalsSql, [
			filter.effectiveStart,
			filter.effectiveEnd,
		]);
		...
		for (const s of signals) {
			...
			if (s.raw_value !== null)
				entry.raw_values[s.component_name] = s.raw_value;
			if (s.signal_direction !== null)
				entry.signal_directions[s.component_name] = s.signal_direction;
		}
      ...
      LIMIT ?
    `;
		const dailyRows = executeQuery<any>(dailySql, [
			filter.effectiveStart,
			filter.effectiveEnd,
			filter.effectiveLimit,
		]);
...
lttdRouter.get("/onchain", (c) => {
	try {
		...
		const COMPONENT_MAP: Record<
			string,
			"sth_mvrv" | "sth_nupl" | "sth_sopr_24h"
		> = {
			mvrv_z: "sth_mvrv",
			aviv_nupl: "sth_nupl",
    ...
    `;
		...
		for (const r of rows) {
			...
			else if (dataSource === "empty" && r.system_source === "VALUATION")
				dataSource = "valuation-fallback";
		}
	...
	try {
		...
		if (typeof (globalThis as any).Bun !== "undefined") {
			...
			const timeoutPromise = new Promise<never>((_, reject) =>
				setTimeout(
					() => reject(new Error("Action timed out after 60s")),
					60000,
				),
			);

			const exitCode = await Promise.race([proc.exited, timeoutPromise]).catch(
				async (err) => {
					proc.kill();
					throw err;
				},
...
lttdRouter.get("/backtest", (c) => {
	try {
		...
		const validatedFeeBps =
			isNaN(feeBps) || feeBps < 0 ? 10 : Math.min(feeBps, 100);
    ...
    `;
		...
		// CAGR
		const cagrStrat =
			years > 0 && equity > 0 ? (equity ** (1 / years) - 1) * 100 : 0;
		const cagrMarket =
			years > 0 && marketEquity > 0
				? (marketEquity ** (1 / years) - 1) * 100
				: 0;

		// Annualized return
		const annReturnStrat = years > 0 ? (equity ** (1 / years) - 1) * 100 : 0;
		const annReturnMarket =
			years > 0 ? (marketEquity ** (1 / years) - 1) * 100 : 0;

		// Annualized volatility
		const meanDailyReturn =
			n > 0 ? dailyReturns.reduce((a, b) => a + b, 0) / n : 0;
		const meanMarketReturn =
			n > 0 ? marketReturns.reduce((a, b) => a + b, 0) / n : 0;
		const stratVariance =
			n > 0
				? dailyReturns.reduce((a, b) => a + (b - meanDailyReturn) ** 2, 0) / n
				: 0;
		const marketVariance =
			n > 0
				? marketReturns.reduce((a, b) => a + (b - meanMarketReturn) ** 2, 0) / n
				: 0;
		const annVolatilityStrat = Math.sqrt(stratVariance) * Math.sqrt(365) * 100;
		const annVolatilityMarket =
			Math.sqrt(marketVariance) * Math.sqrt(365) * 100;

		// Sharpe ratio (risk-free rate = 0)
		const sharpeRatio =
			annVolatilityStrat > 0 ? annReturnStrat / annVolatilityStrat : 0;
		const sharpeRatioMarket =
			annVolatilityMarket > 0 ? annReturnMarket / annVolatilityMarket : 0;
		...
		const sortinoRatio =
			Math.sqrt(downsideVariance) > 0
				? (meanDailyReturn * 365) /
					(Math.sqrt(downsideVariance) * Math.sqrt(365))
				: 0;
