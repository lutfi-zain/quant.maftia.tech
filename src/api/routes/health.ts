...
healthRouter.get('/', (c) => {
  try {
    const maxRow = executeQuerySingle<{ max_date: string }>('SELECT MAX(date) as max_date FROM unified_daily_analytics')
    const countRow = executeQuerySingle<{ count: number }>('SELECT COUNT(*) as count FROM unified_daily_analytics')
