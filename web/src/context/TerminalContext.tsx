...
export const TerminalProvider: React.FC<{ children: React.ReactNode }> = ({
	...
	const [circuitBreakers, setCircuitBreakers] =
		useState<CircuitBreakersResponse | null>(null);
	const [isLoading, setIsLoading] = useState<boolean>(true);
	const [error, setError] = useState<string | null>(null);
	const [syncGap, setSyncGap] = useState<SyncGap>({
		serverDate: null,
		clientDate: null,
		gapDays: 0,
	});
	...
	const handleAnalyticsUpdate = useCallback((newPoint: any) => {
		const mappedPoint: DailyAnalyticsPoint = {
			...
			date: newPoint.date,
			open:
				newPoint.master_ohlcv?.open ??
				(typeof newPoint.open === "number" ? newPoint.open : 0),
			high:
				newPoint.master_ohlcv?.high ??
				(typeof newPoint.high === "number" ? newPoint.high : 0),
			low:
				newPoint.master_ohlcv?.low ??
				(typeof newPoint.low === "number" ? newPoint.low : 0),
			close:
				newPoint.master_ohlcv?.close ??
				(typeof newPoint.close === "number" ? newPoint.close : 0),
			volume:
				newPoint.master_ohlcv?.volume ??
				(typeof newPoint.volume === "number" ? newPoint.volume : 0),
			valuation_composite:
				newPoint.valuation_composite?.score ??
				(typeof newPoint.valuation_composite === "number"
					? newPoint.valuation_composite
					: 0),
			lttd_regime:
				newPoint.lttd_regime?.regime ??
				(typeof newPoint.lttd_regime === "string"
					? newPoint.lttd_regime
					: "SIDEWAYS"),
			...
			mttd_imo:
				newPoint.mttd_imo?.oscillator ??
				(typeof newPoint.mttd_imo === "number" ? newPoint.mttd_imo : 0),
			mttd_er_ratio: newPoint.mttd_imo?.efficiency_ratio ?? 0,
			mttd_shannon_entropy: newPoint.mttd_imo?.shannon_entropy ?? 0,
			ichimoku_imo:
				newPoint.ichimoku_imo?.oscillator ??
				(typeof newPoint.ichimoku_imo === "number" ? newPoint.ichimoku_imo : 0),
		...
		setDailyData((prev) => {
			...
			return [...prev, mappedPoint].sort((a, b) =>
				a.date.localeCompare(b.date),
			);
		});
	}, []);

	const handleCircuitBreakerTrip = useCallback(
		(update: CircuitBreakersResponse) => {
	...
	const refreshData = useCallback(async () => {
		try {
			...
			try {
				...
				const clientDate =
					analytics.length > 0 ? analytics[analytics.length - 1].date : null;
				const gapDays =
					serverDate && clientDate ? daysDiff(clientDate, serverDate) : 0;
