...
export const quantClient = {
	...
	async postSdcaBacktest(params?: any): Promise<any> {
		const res = await fetch(`${API_BASE}/api/v1/sdca/backtest`, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify(params || {}),
		});
		if (!res.ok) throw new Error(`SDCA backtest recalculation failed: ${res.statusText}`);
		return res.json();
	},
	...
	async getDailyAnalytics(limit?: number): Promise<DailyAnalyticsPoint[]> {
		...
		const mapped: DailyAnalyticsPoint[] = rawList.map((item) => ({
			...
			ichimoku_wave_type: item.ichimoku_imo?.wave_type ?? undefined,
			ichimoku_target_v: item.ichimoku_imo?.target_v ?? undefined,
			ichimoku_target_n: item.ichimoku_imo?.target_n ?? undefined,
			ichimoku_target_e: item.ichimoku_imo?.target_e ?? undefined,
			ichimoku_target_nt: item.ichimoku_imo?.target_nt ?? undefined,
			ichimoku_kairitsu: item.ichimoku_imo?.kairitsu ?? undefined,
			ichimoku_cloud_thickness: item.ichimoku_imo?.cloud_thickness ?? undefined,
			ichimoku_kihon_score: item.ichimoku_imo?.kihon_score ?? undefined,
			ichimoku_kumo_twist_flag: item.ichimoku_imo?.kumo_twist_flag ?? undefined,
