...
export const quantClient = {
	...
	async getDailyAnalytics(limit?: number): Promise<DailyAnalyticsPoint[]> {
		const url = new URL(
			`${API_BASE}/api/v1/analytics/daily`,
	...
	async getComponents(
		...
		const url = new URL(
			`${API_BASE}/api/v1/analytics/components`,
			window.location.origin,
		);
	...
	async getMetricTimeseries(
		...
		const url = new URL(
			`${API_BASE}/api/v1/analytics/metric/${encodeURIComponent(metricName)}`,
			window.location.origin,
		);
	...
	async getMetricConfig(metricName: string): Promise<MetricThresholdConfig> {
		const res = await fetch(
			`${API_BASE}/api/v1/analytics/metric/${encodeURIComponent(metricName)}/config`,
		);
		if (!res.ok)
