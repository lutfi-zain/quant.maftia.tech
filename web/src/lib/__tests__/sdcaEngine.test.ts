...
describe("sdcaMultiplier", () => {
	it("returns -0.5x for euphoria (composite >= +1.5)", () => {
		expect(sdcaMultiplier(1.6)).toBe(-0.5);
		expect(sdcaMultiplier(2.0)).toBe(-0.5);
		expect(sdcaMultiplier(10)).toBe(-0.5);
	});
	...
	it("returns 3.0x for deep discount (composite <= -1.5)", () => {
		...
		expect(sdcaMultiplier(-1.7)).toBe(3.0);
...
describe("computeSdcaSignal", () => {
	it("computes full signal for a given day", () => {
		const data: DailyRecord[] = Array.from({ length: 60 }, (_, i) => ({
			...
			close: 50000 + i * 100,
			valuation_composite: -1.2 + i * 0.02, // Rising from deep discount toward fair
		}));
