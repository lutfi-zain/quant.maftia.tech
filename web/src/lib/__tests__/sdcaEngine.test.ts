...
describe("determineAction", () => {
	it("START_AGGRESSIVE_DCA on entry signal", () => {
	...
	it("REDUCE_POSITION on extended euphoria (> 30 days)", () => {
...
describe("computeSdcaSignal", () => {
	it("computes full signal for a given day", () => {
		const data: DailyRecord[] = Array.from({ length: 60 }, (_, i) => ({
			...
			close: 50000 + i * 100,
			valuation_composite: -1.2 + i * 0.02, // Rising from deep discount
		}));
	...
	it("enforces t-1 causal filtering", () => {
		const data: DailyRecord[] = [
			{ date: "2024-01-01", close: 50000, valuation_composite: -1.5 },
