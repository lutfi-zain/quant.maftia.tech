/**
 * Client-side copy of mapToOscillator for the Valuation Studio
 * metric detail chart. Maps a raw metric value to [-2.0, +2.0] using
 * 5 piecewise-linear threshold points with auto-detected direction.
 */
...
export function mapToOscillator(rawValue: number, t: ThresholdConfig): number {
	if (rawValue === null || isNaN(rawValue) || !isFinite(rawValue)) return NaN;

	const { t_minus_2, t_minus_1, t_zero: _t_zero, t_plus_1, t_plus_2 } = t;
