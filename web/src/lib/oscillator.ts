...
export function mapToOscillator(
	...
	if (!inverted) {
		...
		if (rawValue >= t_plus_2 && rawValue < t_plus_1) {
			return 2.0 - safe_div(rawValue - t_plus_2, t_plus_1 - t_plus_2);
		}
		if (rawValue >= t_plus_1 && rawValue < t_minus_1) {
	...
	if (rawValue > t_plus_1 && rawValue <= t_plus_2) {
		return 1.0 + safe_div(rawValue - t_plus_1, t_plus_2 - t_plus_1);
	}
	if (rawValue > t_minus_1 && rawValue <= t_plus_1) {
