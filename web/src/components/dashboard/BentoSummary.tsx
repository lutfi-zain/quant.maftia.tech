...
export const BentoSummary: React.FC<BentoSummaryProps> = ({
	...
	const lttdRegime =
		...
		latestPoint.lttd_regime !== null
			...
			: latestPoint.lttd_regime || circuitBreakers.lttd_macro_override.regime;
	const isSidewaysOverride =
		lttdRegime === "SIDEWAYS" ||
		circuitBreakers.lttd_macro_override.is_sideways_override;

	const mttdImo = toNum(latestPoint.mttd_imo);
	const { er_gate_open, shannon_entropy_gate_open } =
	...
	return (
		...
		>
			...
			>
				...
				>
					Unified 4-System Quantitative Defense &amp; High-End Financial
					Terminal.
				</h2>
				...
				>
					Macroeconomic piecewise linear interpolation, 3-State Gaussian HMM
					regime filters, and multi-principle stationary consensus.
				</p>
			...
			<div ref={bentoRef} className="bento-dense-grid">
				...
				>
					<div>
						...
						>
							Piecewise linear interpolation bounded to [-2.00, +2.00]. Acts as
							the canonical primary macro circuit breaker when score crosses
							bubble threshold.
						</p>
					</div>
					...
					>
						{isSidewaysOverride ? (
							...
							<>
								...
								>
									Exposure Multiplier:{" "}
									{circuitBreakers.lttd_macro_override.exposure_multiplier *
										100}
									%
								</span>
							</>
					...
					>
						...
						>
							...
							<span
								style={{
									fontWeight: 600,
									color: shannon_entropy_gate_open
										? "var(--status-success)"
										: "var(--status-danger)",
								}}
							>
								{shannon_entropy_gate_open ? "PASSED" : "HIGH NOISE"}
							</span>
						</div>
						<div
							style={{
					...
					<div>
						...
						<div
							style={{
								...
								fontFamily: "Geist Mono, JetBrains Mono, monospace",
								color:
									ichimokuImo > 0 ? "var(--accent)" : "var(--status-danger)",
								letterSpacing: "-0.8px",
							}}
						>
							{ichimokuImo > 0
								? `+${ichimokuImo.toFixed(4)}`
								: ichimokuImo.toFixed(4)}
						</div>
						<div
							style={{
			...
			<div className="marquee-container">
				<div className="marquee-track">
					{[...statisticalFamilies, ...statisticalFamilies].map(
						(family, idx) => (
							<span
								key={idx}
								style={{
			...
			<div style={{ width: "100%", maxWidth: "1152px", margin: "0 auto" }}>
				...
				>
					<div>
						...
						>
							Hover or click any system slice to expand architectural
							mathematics & access sandbox
						</div>
					</div>
				...
				<div className="accordion-slice-panel">
					...
					>
						<div>
							...
							{activeAccordion === "valuation" && (
								...
								>
									Macroeconomic valuation score piecewise linear interpolated
									into [-2.0, +2.0] across 17 Fundamental, Technical, and
									Sentiment metrics. Governs long-term structural bubble risk
									(&ge; +1.50) vs deep value accumulation (&le; -1.00).
								</p>
							)}
						</div>
					...
					>
						<div>
							...
							>
								State: {lttdRegime} (
								{circuitBreakers.lttd_macro_override.exposure_multiplier * 100}
								%)
							</div>
							{activeAccordion === "lttd" && (
								<p
								...
								>
									Orthogonal trend classification via 3-State Gaussian Hidden
									Markov Model (BULL, BEAR, SIDEWAYS). When P(Sideways) &gt;
									0.60, forces strict 0.0 exposure override to prevent chop
									degradation on mid-term strategies.
								</p>
							)}
						</div>
					...
					>
						<div>
							...
							{activeAccordion === "mttd" && (
								...
								>
									Multi-principle consensus oscillator [-1.0, +1.0] across 10
									Statistical Families. Governed strictly by three logic gates:
									Efficiency Ratio (ER &ge; 0.20), Shannon Entropy (H &le;
									2.30), and Chikou Momentum Exit.
								</p>
							)}
						</div>
						...
						<div>
							...
							<div
								style={{
									fontSize: "13px",
									fontWeight: 700,
									color: ichimokuImo > 0 ? "#3B82F6" : "var(--status-danger)",
									fontFamily: "Geist Mono, JetBrains Mono, monospace",
									marginTop: "6px",
								}}
							>
							...
							{activeAccordion === "ichimoku" && (
								...
								>
									Stationary bounded tanh oscillator transforming non-stationary
									cloud components (S_TK, S_Cloud, S_Future, S_Chikou) filtered
									through Ehlers 2-pole SuperSmoother IIR transfer function for
									zero-lag noise removal.
								</p>
							)}
						</div>
