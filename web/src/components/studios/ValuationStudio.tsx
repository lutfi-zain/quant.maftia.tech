...
> = {
  ...
  'RHODL Ratio': {
    category: 'Fundamental',
    description:
      'Realized HODL ratio weighting recent coin age over old coins',
  },
  'SOPR (90d SMA)': {
    category: 'Fundamental',
  ...
  'Reserve Risk': {
    category: 'Fundamental',
    description:
      'Confidence of long-term holders vs price incentive to sell',
  },
  'Thermocap Multiple': {
    category: 'Fundamental',
    description:
      'Market capitalization divided by cumulative miner revenue',
  },
  'NVT Golden Cross': {
    category: 'Fundamental',
    description:
      'Network Value to Transactions ratio short vs long trend',
  },
  ...
  '200-Week SMA Heatmap': {
    category: 'Technical',
    description:
      'Percentage distance above canonical 200-week moving average',
  },
  'RSI (14-Month)': {
    category: 'Technical',
  ...
  'Bollinger Band Width (Log)': {
    category: 'Technical',
    description:
      'Standard deviation compression and breakout volatility',
  },
  'Mayer Multiple': {
    category: 'Technical',
  ...
  'Fear & Greed Index (30d SMA)': {
    category: 'Sentiment',
    description:
      'Multi-factor social & market sentiment composite',
  },
  'Net Unrealized Profit/Loss (NUPL)': {
    category: 'Sentiment',
    description:
      'Total market unrealized profit versus loss proportion',
  },
  'Exchange Net Flow Velocity': {
    category: 'Sentiment',
    description:
      'Aggregated net exchange inflows/outflows momentum',
  },
  'Miner Outflow Ratio': {
    category: 'Sentiment',
    description:
      'Proportion of miner wallet transfers to liquid exchanges',
  },
};
...
export const ValuationStudio: React.FC = () => {
    ...
    allCharts.forEach(({ chart, series }, idx) => {
      chart.subscribeCrosshairMove((param) => {
        ...
        if (param.time) {
          ...
          setHoveredPoint(
            dailyData.find((p) => p.date === timeStr) || null,
          );
  ...
  const toNum = (val: any): number =>
    typeof val === 'object' && val !== null
      ? Number(
          val.score ?? val.oscillator ?? val.normalized_score ?? 0,
        )
      : Number(val ?? 0);
            ...
            >
              <div className="chart-subplot-header">
                <span
                  ...
                  style={{ color: 'var(--text-dim)' }}
                >
                  Valuation Composite [-2.00 → +2.00] · Bubble +1.50 /
                  Discount -1.00
                </span>
                <div className="subplot-controls">
                  <span
            ...
            <tbody>
              {displayIndicators.map((ind) => {
                ...
                return (
                  <tr
                    key={ind.name}
                    onClick={() => setSelectedMetric(ind.name)}
                    style={{
                      borderBottom:
