...
export const ValuationStudio: React.FC = () => {
	...
	useEffect(() => {
		...
		if (isMobile && maximized !== null) {
			...
			if (containerH && containerH > 0) {
				const total = heights.btc + heights.val;
				if (total > 0) {
					...
					btc
						.timeScale()
						.applyOptions({ visible: heights.val === 0 });
	...
	useEffect(() => {
		...
		// Valuation Composite Pane (bottom, time axis visible)
