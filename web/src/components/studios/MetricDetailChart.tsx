...
export const MetricDetailChart: React.FC<MetricDetailChartProps> = ({
  ...
  useEffect(() => {
    ...
    if (osc) osc.timeScale().applyOptions({ visible: heights.btc > 0 || heights.raw > 0 });
  }, [maximized]);
  ...
  useEffect(() => {
    const data = responseDataRef.current;
    if (!data || !seriesRef.current.oscLine || !seriesRef.current.rawLine) return;
    ...
    const lineConfigs: { key: keyof typeof priceLinesRef.current; price: number; color: string; title: string }[] = [
      { key: 't_minus_2', price: thresholds.t_minus_2, color: '#EF4444', title: 'Peak -2' },
      { key: 't_minus_1', price: thresholds.t_minus_1, color: '#FB7185', title: 'Distrib -1' },
      { key: 't_zero', price: thresholds.t_zero, color: '#475569', title: '' },
      { key: 't_plus_1', price: thresholds.t_plus_1, color: '#34D399', title: 'Accum +1' },
      { key: 't_plus_2', price: thresholds.t_plus_2, color: '#22C55E', title: 'Bottom +2' },
    ];
  ...
  const panelControls = (label: 'btc' | 'raw' | 'osc') => (
    <div className="subplot-controls">
      ...
      <button
        className="icon-btn"
        onClick={() =>
          setMaximized(maximized === label ? null : label)
        }
    ...
    <div className="glass-card" style={{ padding: '16px' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <button className="icon-btn" onClick={onClose} title="Close detail view">
            ✕
          </button>
        </div>
      </div>
      ...
      >
        ...
        >
          <div className="chart-subplot-header">
            <span className="subplot-title" style={{ color: 'var(--text-dim)' }}>
              BTC USD Candlestick
            </span>
            {panelControls('btc')}
          </div>
