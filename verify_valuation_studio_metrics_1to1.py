...
def simulate_studio_backtest_ts(data_rows, start_date, end_date, fee_bps=10):
    ...
    # Close open trade if still active at end
...
def main():
    print("==========================================================================")
    print(" VALUATION STUDIO METRICS 1:1 PARITY VERIFICATION HARNESS")
    print("==========================================================================")
    ...
    with get_wal_connection(db_path) as conn:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT date, btc_price, valuation_composite
    ...
    for r in rows:
        ...
        pos = 1 if val_num < 1.5 else 0
        data_rows.append({
            'date': r[0],
            'close': float(r[1]),
            'position': pos,
            'valuation_composite': val_num
        })
    ...
    print(f"\n--- Verifying Valuation Studio Output ({start_date} -> NOW) ---")
    ...
    print("\n==========================================================================")
    print(" SUMMARY: Valuation Studio 1:1 Parity Assertions Passed Cleanly!")
    print("==========================================================================")
