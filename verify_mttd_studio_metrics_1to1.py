...
def main():
    print("==========================================================================")
    print(" MTTD STUDIO METRICS 1:1 PARITY VERIFICATION HARNESS")
    print("==========================================================================")
    ...
    with get_wal_connection(db_path) as conn:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT date, btc_price, mttd_imo, mttd_er, mttd_entropy
    ...
    data_rows = []
    for r in rows:
        imo = float(r[2]) if r[2] is not None else 0.0
        er = float(r[3]) if r[3] is not None else 0.0
        entropy = float(r[4]) if r[4] is not None else 0.0
        pos = 1 if (imo > 0.15 and er >= 0.20 and entropy <= 2.30) else 0
        data_rows.append({
            'date': r[0],
            'close': float(r[1]),
            'position': pos,
    ...
    print(f"\n--- Verifying MTTD Console Output ({start_date} -> NOW) ---")
    ...
    print("\n==========================================================================")
    print(" SUMMARY: MTTD Studio 1:1 Parity Assertions Passed Cleanly!")
    print("==========================================================================")
